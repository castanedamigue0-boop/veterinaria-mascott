import { crearUsuario, obtenerUsuario, actualizarUsuario, setSession, getSession } from '../scripts/base-de-datos.js';

// Si ya hay sesión → dashboard
if (getSession()) window.location.href = '../paginas/panel-usuario.html';
...
document.getElementById('loginForm').addEventListener('submit', async e => {
  ...
  try {
    ...
    setTimeout(() => { window.location.href = redirect === 'cita' ? 'inicio.html#inicio' : '../paginas/panel-usuario.html'; }, 700);
  } catch(err) {
    msg.className = 'form-msg error';
...
document.getElementById('registerForm').addEventListener('submit', async e => {
  ...
  try {
    ...
    const redirect = sessionStorage.getItem('macott_redirect');
    sessionStorage.removeItem('macott_redirect');
    setTimeout(() => { window.location.href = redirect === 'cita' ? 'inicio.html#inicio' : '../paginas/panel-usuario.html'; }, 900);
  } catch(err) {
    msg.className = 'form-msg error';
