...
function renderProductos() {
  ...
  grid.innerHTML = filtrados.map(p => {
    ...
    return '<article class="producto-card' + (agotado ? ' producto-agotado' : '') + '" role="listitem" data-id="' + p.id + '">'
      ...
      + '<div><div class="producto-precio">$' + p.precio + ' COP</div>' + oldHtml + '</div>'
...
function renderCarritoPanel() {
  ...
  if (!carrito.length) {
    ...
    $('carritoSubtotal').textContent = '$0 COP';
  ...
  body.innerHTML = carrito.map(c => {
    ...
    return '<div class="carrito-item">'
      ...
      + '<span class="ci-precio">$' + (p.precio * c.qty) + ' COP</span>'
      + '<div class="ci-qty">'
  ...
  $('carritoSubtotal').textContent = '$' + total + ' COP';
...
function abrirCheckout() {
  ...
  $('checkoutContent').innerHTML = '<div class="checkout-form">'
    ...
    + '<div class="checkout-total"><span>Total</span><span>$' + total + ' COP</span></div></div>'
    ...
    + '</div>'
    + '<button class="btn-pagar" id="btnPagar">✅ Confirmar pedido — $' + total + ' COP</button>'
    + '</div>';
...
function initSesion() {
  ...
  if (session && loginBtn) {
    ...
    loginBtn.href = '../paginas/panel-usuario.html';
  }
}
...
function abrirModalProducto(id) {
  ...
  document.getElementById('modalProdPrecio').textContent = '$' + p.precio + ' COP';
